...
pendingTimers=[];}
function buildBoard(){boardEl.innerHTML="";reels=[];cells=[];for(var col=0;col<REEL_COUNT;col+=1){var reel=document.createElement("div");reel.className="jade-reel-91650591";var columnCells=[];for(var row=0;row<ROW_COUNT;row+=1){var cell=document.createElement("div");cell.className="jade-cell-91650591";var glyph=document.createElement("span");glyph.className="jade-glyph-91650591";glyph.textContent=drawSymbol().glyph;cell.appendChild(glyph);reel.appendChild(cell);columnCells.push(cell);}
...
function clearMarks(){for(var col=0;col<REEL_COUNT;col+=1){reels[col].classList.remove("jade-shimmer","jade-turning");for(var row=0;row<ROW_COUNT;row+=1){cells[col][row].classList.remove("jade-hit-91650591");}}
if(cabinetEl){cabinetEl.classList.remove("jade-blessed-91650591");}
var keys=Object.keys(payChips);for(var i=0;i<keys.length;i+=1){payChips[keys[i]].classList.remove("jade-hit-91650591");}}
function buildPayStrip(){payStripEl.innerHTML="";payChips={};for(var i=0;i<SYMBOLS.length;i+=1){var sym=SYMBOLS[i];var chip=document.createElement("li");chip.className="jade-pay-chip-91650591";chip.title="Three "+sym.label+" on the middle line";var glyphs=document.createElement("span");glyphs.className="jade-pay-glyphs-91650591";glyphs.textContent=sym.glyph+sym.glyph+sym.glyph;var pay=document.createElement("span");pay.className="jade-pay-x-91650591";pay.textContent="×"+sym.pay;chip.appendChild(glyphs);chip.appendChild(pay);payStripEl.appendChild(chip);payChips[sym.key]=chip;}}
...
tell("Two golden tigers hold the line — free respin!","");reels[oddCol].classList.add("jade-shimmer-91650591");setTurning(oddCol,true);later(function(){state.grid[oddCol]=drawColumn();setTurning(oddCol,false);reels[oddCol].classList.remove("jade-shimmer-91650591");showColumn(oddCol,state.grid[oddCol]);later(judgeRound,220);},RESPIN_MS);}
function settleWin(symbol){var prize=Math.round(state.stake*symbol.pay*100)/100;state.coins=Math.round((state.coins+prize)*100)/100;saveCoins();showCoins();for(var col=0;col<REEL_COUNT;col+=1){cells[col][CENTER_ROW].classList.add("jade-hit-91650591");}
if(payChips[symbol.key]){payChips[symbol.key].classList.add("jade-hit-91650591");}
if(cabinetEl){cabinetEl.classList.add("jade-blessed-91650591");}
